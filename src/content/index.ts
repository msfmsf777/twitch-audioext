...
class AudioEngine {
  ...
  private wasmLoading: Promise<ArrayBuffer> | null = null;
  ...
  private async loadWorklet(): Promise<void> {
    ...
    try {
      ...
      const wasmBytes = await this.getWasmBytes();
  ...
  private async getWasmBytes(): Promise<ArrayBuffer> {
    ...
    if (!this.wasmLoading) {
      this.wasmLoading = fetch(this.wasmUrl)
        .then((response) => {
          if (!response.ok) {
            throw new Error(`Failed to fetch Rubber Band WASM: ${response.status}`);
          }
          return response.arrayBuffer();
        })
        .then((buffer) => {
          this.wasmBytes = buffer;
          return buffer;
        })
        .catch((error) => {
